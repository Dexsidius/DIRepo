...
if __name__ == "__main__":
    ...
    ## Sprite Cache stores sprites so that they aren't loaded onto memory multiple times
    #  the cache allows you to load textures once as a result.
    class SpriteCache:
        def __init__(self, renderer):
            self._cache = {}  #The dictionary where the sprited are stored
            self._renderer = renderer

        def loadTexture(self, file):
            # if the file is not in the dictionary, it adds it and stores it based on the filename
            if file not in self._cache:
                surface = SDL_LoadBMP(file.encode('utf-8'))
                self._cache[file] = SDL_CreateTextureFromSurface(self._renderer, surface)
        ...
        def Quit(self):
            for key in self._cache:
                SDL_DestroyTexture(self._cache[key])
    ...
    def main():
        ...
        window = SDL_CreateWindow(b"RPG GAME By Isa and Da'Shawn", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                         WIDTH, HEIGHT, 0)
        renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_PRESENTVSYNC)
        SDL_ShowCursor(0)

        cache = SpriteCache(renderer)
        ...
        class Background:
            def __init__(self, w, h, x, y):
                ...
                self.bgTexture = cache.loadTexture(self.path)

            def Render(self):
                SDL_RenderCopy(renderer, self.bgTexture, None, None)
        ...
        class AnimatedCharacter:
            def __init__(self, w, h, x, y):
                ...
                self.path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                         "content", "others.bmp")
                self.texture = cache.loadTexture(self.path)
